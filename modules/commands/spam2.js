...
module.exports.run = async function({ api, event, args }) {
  ...
  if (numberOfSpams > 500 || numberOfSpams < 1) {
    api.sendMessage(`Số lần spam không được quá 500 lần và phải lớn hơn 0`, event.threadID, event.messageID);
    return;
  }
  ...
  let spamCount = 0;
  let errorCount = 0;
  api.sendMessage(`Đang Bắt đầu spam ${numberOfSpams} lần\ncho số điện thoại ${phoneNumber}`, event.threadID);
  for (let i = 0; i < numberOfSpams && isSpamming; i++) {
    await delay(2000);
    try {
      const response = await axios.get(`https://spam.zgzggdsg.repl.co/?phone=${phoneNumber}`, { responseType: 'arraybuffer' });
